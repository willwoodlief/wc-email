...
if ( !function_exists( 'woo_mb_remove_single_email_template' ) ) {
    function woo_mb_remove_single_email_template($language, $template)
    {
        global $wpdb;
        $status = $wpdb->get_var("DELETE FROM $wpdb->posts WHERE `post_type` = 'woo_mb_template' AND `post_name` = '".sanitize_text_field($language)."' AND `post_title` = '".sanitize_text_field($template)."'");
        return $status;
    }
}